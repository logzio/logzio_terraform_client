...
- `SearchTerm` matches **only `metadata.name`** — it is not a full-text search. A dashboard whose display name, panel names, PromQL queries and datasource references all contained a distinctive token was not returned for any of them, only for its `metadata.name` (probed 2026-08-20). There is no endpoint on this gateway that finds dashboards by the datasource they reference.
